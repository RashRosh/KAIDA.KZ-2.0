...
// A read projection across the four owning modules; no writes or lifecycle rules.
export async function findOffersByProductName(db: Database, query: string): Promise<SearchOffer[]> {
  const rows = await db.select({
    ...
    .where(sql`lower(${products.name}) = lower(${query})`)
