...
4. **Неполная карточка публикуется, продавец получает напоминание** (решение 2026-09-25, отменяет прежнее «фото
   обязательно»). Всё публикуется из черновика. Без фото, комментария и другого необязательного содержимого карточка
   публикуется, но перед публикацией и на «Моей витрине» продавец видит напоминание, что карточка неполная и будет
   проигрывать конкурентам, с действием дополнить её. Публикацию блокируют только данные, без которых карточка не
   работает: название, цена, «Цена за» и точка. Отсутствие фото на порядок выдачи **не влияет** — у покупателя
...
12. **Актуальность и ИИ — обязательны к запуску сервиса** (решение 2026-09-25): это главные отличия KAIDA, без них
    ...
    входит в этап 1 вместе с «Моей витриной»; ИИ-ввод и ИИ-модерация идут отдельными этапами, но обязательно до запуска.
13. **Режим работы точки** (решение 2026-09-25). У точки есть необязательный режим работы; новая точка получает режим
    предыдущей, как контакты. Покупатель видит пометку «Открыто до 19:00» / «Закрыто · откроется в 8:00»; если режим
    не указан, про часы ничего не пишется. Режим работы **не меняет** порядок выдачи и не скрывает карточки. В макете
    режима работы нет — нужна доработка дизайна до контракта.
