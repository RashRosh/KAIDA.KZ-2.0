...
13. **Режим работы точки — обязателен** (решение 2026-09-25). Без режима точку не сохранить; новая точка получает
    ...
    **красный** — закрыто. По `PROJECT_RULES.md` §18.4 состояние не передаётся только цветом — к строке добавляется
    короткое слово или значок (формулировка — в контракте). Режим работы **не меняет** порядок выдачи и не скрывает
