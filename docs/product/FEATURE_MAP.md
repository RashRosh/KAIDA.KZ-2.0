...
4. **Неполная карточка публикуется, продавец получает напоминание** (решение 2026-09-25, отменяет прежнее «фото
   обязательно»). Всё публикуется из черновика. **Неполные данные — это отсутствие фото, комментария и режима работы
   точки** (п. 13): без них карточка публикуется, но перед публикацией и на «Моей витрине» продавец видит
