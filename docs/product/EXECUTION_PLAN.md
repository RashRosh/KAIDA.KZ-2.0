...
| 6 | **В работе.** Localization разделён на три контракта, все **APPROVED** Product Owner 2026-09-23: `localization-foundation` → `catalog-localization` → `seller-comment-translation` (порядок реализации тот же). #27 разложен на три seller UI контракта (**DRAFT**, ждут Product Owner): `seller-cabinet-overview` → `seller-offer-editor` → `seller-points-contacts`; старт после `localization-foundation`. Открытые решения — `seller-offer-editor` §8. GitHub Issue #27 ещё содержит устаревшее направление «без отдельной страницы подтверждения», противоречащее принятому `S-10`; buyer surfaces `S-01`–`S-03` пока без контрактов |
| 7–8 | Не начаты |
