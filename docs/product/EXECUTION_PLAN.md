...
2. **Контакты у точки** — телефон / WhatsApp / Telegram принадлежат точке; новая точка получает контакты предыдущей;
   ...
   **режим работы точки** — необязательный, копируется от предыдущей точки, покупатель видит «Открыто до …» /
   «Закрыто · откроется в …», на порядок выдачи не влияет. Перед контрактом нужен кадр дизайнера (в макете его нет).
