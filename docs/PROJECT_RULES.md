...
Пункты выше о защите business/data contracts от вайрфрейма **отменены и заменены §18.2 ниже** (Product Owner decision, 2026-09-22, тот же день).

### 18.2 Вайрфрейм — абсолютный приоритет над всем ранее написанным (Product Owner decision, 2026-09-22, эскалация §18.1)

Product Owner явно расширил статус bundled wireframe artifact: это больше не только presentation-цель. Это **главный источник истины продукта**, выше по приоритету, чем любой ранее закрытый contract, документ или существующая реализация.

- Если что-либо ранее написанное — closed contract (включая auth, ownership, persistence, pricing, ChangeSet-архитектуру и другие business/data boundaries из раздела 4), код, тест, старый Slice Contract или любой другой документ — мешает реализовать то, что показывает вайрфрейм, то переделывается, ломается, удаляется или корректируется именно это старое, а не вайрфрейм.
- Обязательный `STOP`-процесс из раздела 4 и прежний пункт §18.1 про «explicit вопрос Product Owner перед реализацией новой механики» для конфликтов с вайрфреймом **отменяются**. Новая механика, которую явно показывает вайрфрейм, реализуется напрямую.
- Расхождение с закрытым contract по-прежнему **фиксируется** в Slice Contract соответствующей области (что именно изменилось и какой contract это затрагивает) — это для истории и для Controller, а не запрос на разрешение.
- Если по конкретному экрану остаётся реальная неоднозначность (вайрфрейм можно трактовать двумя разными способами, а не «вайрфрейм против старого contract») — по умолчанию выбирается трактовка, ближе всего соответствующая вайрфрейму, без ожидания ответа Product Owner, если только расхождение не относится к необратимому/деструктивному действию (см. общий git-safety-protocol: force-push, необратимые миграции с потерей данных и т. п. — те по-прежнему подтверждаются отдельно, это не product-исключение, а общая операционная безопасность).
- Это не отменяет `PROJECT_RULES.md` §3 (Slice Contract обязателен для каждого slice) и §20 (не писать весь MVP одним заходом по большому ТЗ) — вайрфрейм определяет **целевое поведение**, а не заменяет процесс маленьких проверяемых vertical slices.
