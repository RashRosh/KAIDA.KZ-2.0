...
test('S10 Seller contacts reach Buyer OfferCard safely and one cleared channel disappears', async ({ page }, testInfo) => {
  ...
  try {
    ...
    await page.getByLabel('Телефон', { exact: true }).fill('8 (700) 123-45-67');
    await page.getByLabel('WhatsApp', { exact: true }).fill('+447911123456');
    await page.getByLabel('Telegram', { exact: true }).fill(`@${telegram.toUpperCase()}`);
    await page.getByLabel('Instagram', { exact: true }).fill(`@${instagram.toUpperCase()}`);
    ...
    await expect(card.getByRole('link', { name: 'Позвонить', exact: true })).toHaveAttribute('href', 'tel:+77001234567');
    ...
    await expect(updatedCard.getByRole('link', { name: 'Позвонить', exact: true })).toHaveAttribute('href', 'tel:+77001234567');
