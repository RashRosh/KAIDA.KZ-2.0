...
async function withUpgradeDatabase<T>(run: (pool: Pool, db: ReturnType<typeof drizzle>) => Promise<T>): Promise<T> {
  ...
  const name = 'kaida_s10_upgrade_test';
  if (decodeURIComponent(developmentUrl.pathname) === `/${name}`) throw new Error('Development database must never be S10 upgrade target');
  ...
  try {
    ...
    if (version < 180000 || version >= 190000) throw new Error('S10 migration test requires PostgreSQL 18');
...
async function migrateToPreS10(db: ReturnType<typeof drizzle>) {
